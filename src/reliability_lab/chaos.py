...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    for breaker in gateway.breakers.values():
        ...
        for log in breaker.transition_log:
            ...
            elif log.get("to") == "closed" and open_ts is not None:
                close_ts = float(log["ts"])
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    for breaker in gateway.breakers.values():
        for log in breaker.transition_log:
            if log.get("to") == "open":
                circuit_opens += 1
    metrics.circuit_open_count = circuit_opens
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        # Evaluate scenario pass/fail condition
        ...
        elif scenario.name == "cascading_provider_outage":
            passed = result.availability >= 0.60
        else:
